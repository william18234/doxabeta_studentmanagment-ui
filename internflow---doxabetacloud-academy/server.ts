...
// Check backend proxy status to http://localhost:8080
...
app.use('/api', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  // First check if localhost:8080 is running
  ...
  if (isUp) {
    try {
      ...
      console.warn('Backend proxy to http://localhost:8080 failed, using mock fallback engine.', e);
    }
  }
  next();
});
