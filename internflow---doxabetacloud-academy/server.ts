...
// Configurable Backend Proxy URL (defaults to live Render server if BACKEND_URL env var is not set)
const BACKEND_URL = (process.env.BACKEND_URL || 'https://doxabeta-student-management-1.onrender.com').replace(/\/$/, '');
const BACKEND_ROOT = BACKEND_URL.endsWith('/api') ? BACKEND_URL.slice(0, -4) : BACKEND_URL;
...
async function checkBackendConnectivity(): Promise<boolean> {
  ...
  try {
    ...
    const healthUrl = `${BACKEND_ROOT}/api/health`;
...
app.use('/api', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  if (isUp) {
    try {
      const targetUrl = `${BACKEND_ROOT}${req.originalUrl}`;
      ...
      console.warn(`Remote backend returned status ${proxyRes.status}. Falling back to mock engine.`);
    } catch (e) {
      console.warn(`Backend proxy to ${BACKEND_ROOT} failed, using mock fallback engine.`, e);
    }
  }
  next();
});

// Mock Backend API implementation (Runs when 8080 is not available)
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    backend8080Active: backendAvailable,
